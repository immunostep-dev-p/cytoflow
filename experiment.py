...
new_experiment_flow_peaks_op = flow_peaks_op.apply(new_experiment)
flow_peaks_op.default_view().plot(new_experiment_flow_peaks_op)
cytoflow.ScatterplotView(xchannel = "R1-A", xscale = "logicle", ychannel = "B8-A", yscale = "logicle", huefacet = "FlowPeaks").plot(new_experiment_flow_peaks_op)

new_experiment_flow_peaks_op.data
# print(new_experiment_flow_peaks.data)

count = new_experiment_flow_peaks_op[["FlowPeaks"]].groupby(by = new_experiment_flow_peaks_op["FlowPeaks"]).count()
count
# print(count)

umbral = abs(new_experiment_flow_peaks_op["R1-A"].max() / new_experiment_flow_peaks_op["R1-A"].min())
umbral
# print(umbral)

indice = new_experiment_flow_peaks_op["R1-A"].loc[new_experiment_flow_peaks_op["R1-A"] < umbral].index
indice
# print(indice)

cluster = new_experiment_flow_peaks_op[["FlowPeaks"]].drop(indice).groupby(by = new_experiment_flow_peaks_op["FlowPeaks"].drop(indice)).count()
cluster
# print(cluster)
...
bead_cluster = new_experiment_flow_peaks_op[new_experiment_flow_peaks_op["FlowPeaks"] == p_cluster]
bead_cluster
# print(bead_cluster)
